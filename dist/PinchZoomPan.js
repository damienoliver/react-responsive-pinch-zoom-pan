...
function (_React$Component) {
  ...
  function PinchZoomPan() {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "scaleForPercent", function (percent) {
      return _this.props.minScale + percent / 100 * (_this.props.maxScale - _this.props.minScale);
    });
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "handleSliderChange", function (value) {
      console.log("slider change to: " + value.value);
      var midpoint = {
        x: _this.state.containerDimensions.width / 2,
        y: _this.state.containerDimensions.height / 2
      };

      var scaleForPercent = _this.scaleForPercent(value.value);

      console.log("new scale: " + scaleForPercent);

      _this.zoom(scaleForPercent, midpoint, 0, 0);
    });
